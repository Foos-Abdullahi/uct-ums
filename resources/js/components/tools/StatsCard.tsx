...
export interface StatSection {
    ...
    badge?: {
        ...
        variant?:
            'blue' | 'green' | 'emerald' | 'red' | 'amber' | 'purple' | 'gray';
    };
    description?: string;
}
...
const badgeVariants: Record<string, string> = {
    ...
    emerald:
        'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-500/10',
...
export function StatsCard({ sections, className = '' }: StatsCardProps) {
    return (
        <div className={`overflow-hidden ${className}`}>
            <div className="group grid cursor-pointer grid-cols-1 divide-y divide-border rounded-sm border-border/50 bg-muted/40 shadow-sm backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:bg-muted/60 md:grid-cols-4 md:divide-x md:divide-y-0 dark:bg-white/[0.03] dark:hover:bg-white/[0.06]">
