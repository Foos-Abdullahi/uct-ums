...
export default function AdminChartOfAccountsImport() {
    ...
    const handlePreview = async () => {
        ...
        try {
            const response = await fetch('/admin/finance/chart-of-accounts/import-preview', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    if (payload?.errors?.file) {
                        message = Array.isArray(payload.errors.file) ? payload.errors.file[0] : payload.errors.file;
                    }
                } catch { /* ignore */ }
            ...
            if (data.errors > 0) {
                toast.error(`${data.errors} row(s) have errors. Fix the workbook before importing.`);
            } else {
                toast.success(`Workbook parsed. ${data.valid + data.warnings} row(s) ready to import.`);
            }
        } catch {
...
return;
        ...
        try {
            const response = await fetch('/admin/finance/chart-of-accounts/import', {
                method: 'POST',
                headers: {
                    Accept: 'application/json',
            ...
            if (!response.ok) {
                toast.error(payload?.message ?? 'Import failed. No changes were saved.');

                return;
            }

            toast.success(payload?.message ?? 'Accounts imported successfully.');
            ...
            <div className="p-6 space-y-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <h1 className="text-lg font-semibold text-foreground tracking-tight">Import Accounts</h1>
                        <p className="text-xs text-muted-foreground mt-0.5">
                            Upload an Excel or CSV workbook, review the validated rows, then import them into the chart of accounts.
                        </p>
                    </div>
                </div>

                <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
                    <span className={preview ? 'text-emerald-600 font-semibold' : 'text-primary font-semibold'}>
                        <CheckCircle2 className="mr-1 inline h-3.5 w-3.5" />1. Upload workbook
                    </span>
                    <span className="text-border">—</span>
                    <span className={preview ? 'text-primary font-semibold' : ''}>
                        <Upload className="mr-1 inline h-3.5 w-3.5" />2. Review & validate
                    </span>
                    <span className="text-border">—</span>
                    <span>3. Import</span>
                </div>
                ...
                <div className="rounded-md border border-border/60 bg-card p-5">
                    <div className="mb-4 flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-sm font-semibold text-foreground">Step 1 — Upload your workbook</h2>
                            <p className="text-xs text-muted-foreground">
                                Expected columns: <span className="font-mono">Code, Name, Group, Type, Normal Balance</span> (optional: Description, Status).
                            </p>
                        </div>
                    </div>
                ...
                {preview && (
                    <div className="rounded-md border border-border/60 bg-card p-5 space-y-4">
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                <h2 className="text-sm font-semibold text-foreground">Step 2 — Review {preview.total} account(s)</h2>
                                <p className="text-xs text-muted-foreground">No accounts are saved until you confirm the import.</p>
                            </div>
                            <div className="flex flex-wrap items-center gap-2">
                                <Badge variant="outline" className="border-emerald-200 bg-emerald-500/10 text-emerald-700">
                                    <CheckCircle2 className="mr-1 h-3 w-3" /> Valid: {preview.valid}
                                </Badge>
                                <Badge variant="outline" className="border-amber-200 bg-amber-500/10 text-amber-700">
                                    <AlertCircle className="mr-1 h-3 w-3" /> Warnings: {preview.warnings}
                                </Badge>
                                <Badge variant={preview.errors > 0 ? 'destructive' : 'outline'}>
                                    <XCircle className="mr-1 h-3 w-3" /> Errors: {preview.errors}
                                </Badge>
                            </div>
                        </div>
                        ...
                        {preview.errors > 0 && (
                            <Alert variant="destructive">
                                ...
                                <AlertDescription className="text-xs">
                                    Fix the {preview.errors} row(s) with errors in the workbook, then review again.
                                </AlertDescription>
                            </Alert>
                        )}
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-border/60 bg-muted/30 text-left text-[10px] uppercase tracking-wider text-muted-foreground">
                                        ...
                                        <th className="hidden px-3 py-2 font-medium md:table-cell">Group</th>
                                ...
                                <tbody className="divide-y divide-border/40">
                                    {preview.rows.map((row) => (
                                        <tr key={row.row} className={row.errors.length > 0 ? 'bg-destructive/5' : ''}>
                                            <td className="px-3 py-2 text-xs text-muted-foreground">{row.row}</td>
                                            <td className="px-3 py-2 font-mono text-xs font-semibold text-foreground">{row.code ?? '—'}</td>
                                            <td className="px-3 py-2">
                                                <p className="text-xs font-medium text-foreground">{row.name || '—'}</p>
                                                <p className="text-[10px] text-muted-foreground">{row.group_name}</p>
                                            </td>
                                            <td className="hidden px-3 py-2 text-xs text-muted-foreground capitalize md:table-cell">{row.group_name}</td>
                                            <td className="hidden px-3 py-2 sm:table-cell">
                                                {row.type ? TYPE_BADGES[row.type] : <span className="text-xs text-muted-foreground">—</span>}
                                            </td>
                                            <td className="px-3 py-2">
                                                {row.errors.length > 0 ? (
                                                    <div className="space-y-1">
                                                        {row.errors.map((error, idx) => (
                                                            <p key={idx} className="text-[10px] text-destructive">{error}</p>
                                                        ))}
                                                    </div>
                                                ) : row.warnings.length > 0 ? (
                                                    <div className="space-y-1">
                                                        {row.warnings.map((warning, idx) => (
                                                            <p key={idx} className="text-[10px] text-amber-700">{warning}</p>
                                                        ))}
                                                    </div>
                                                ) : (
                                ...
                                <Button type="button" size="sm" disabled={importing} onClick={handleImport}>
                                    {importing ? (
                                        ...
                                        <>
                                            ...
                                            Import {preview.valid + preview.warnings} account(s)
                                        </>
                                    )}
                                </Button>
                            </div>
